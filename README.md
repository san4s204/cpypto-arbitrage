...
1. оставляет активные spot-USDT пары, доступные хотя бы на двух выбранных биржах;
2. проверяет минимальный суточный объём на доступных для пары биржах;
...
## Высокочастотная запись синхронных котировок
...
- `runtime/live_latency_routes.csv` — статистика и cost-aware replay направленных маршрутов;
- четыре консольных рейтинга: `LIVE SPREAD`, `MICRO-TREND VOLATILITY`, `LATENCY`
  и `DIRECTED LATENCY ROUTES`.

Пара сохраняется, когда свежие котировки пришли минимум с двух из трёх бирж: отсутствие
монеты на одной площадке больше не выбрасывает её из исследования. Интервал `0.5s` нужен,
чтобы увидеть короткое запаздывание, которое терялось при записи раз в 5 секунд. Такая
запись растёт заметно быстрее, поэтому первый прогон лучше ограничить тремя парами и
2–4 часами.
...
площадок это шесть маршрутов. По умолчанию проверяются lookback `0.5/1s`, движения
лидера `12/20/30 bps`, минимальные разрывы `8/15 bps`, ответы ведомой биржи
`8/15/25 bps` и удержание `1/2/5s`. Результат сохраняется в
`runtime/latency_replay.csv`; строки маршрутов имеют `row_type=route`.
...
Текущая версия `latency_momentum` выключена в YAML до положительной проверки на двух
независимых сессиях.
